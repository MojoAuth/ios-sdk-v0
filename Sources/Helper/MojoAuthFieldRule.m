...
@implementation MojoAuthFieldRule
...
- (MojoAuthFieldRuleType) setTypeWithString:(NSString*)ruleStr
{
    MojoAuthFieldRuleType fieldType = unknown;
...
{
    NSString *MojoAuthfieldstr = [NSString stringWithFormat:@"<MojoAuthFieldRule, type: %@, intValue: %@, stringValue: %@, regexValue:%@",[self typeToString], [self intValue], [self stringValue], [self regex]];
...
{
    return MojoAuthFieldRuleType([self type]);
}
